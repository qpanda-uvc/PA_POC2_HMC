...
# 고정 프리팹 목록 (이 프로젝트에서 사용 가능한 것만)
FIXED_PREFABS = """- Name: 7_ChargeZone | Path: Assets/Prefabs/7_ChargeZone.prefab
- Name: 8_NORMAL_STAGE | Path: Assets/Prefabs/8_NORMAL_STAGE.prefab
- Name: 9_Docking_Station_1_ | Path: Assets/Prefabs/9_Docking_Station_1_.prefab
- Name: 9_Docking_Station_2_ | Path: Assets/Prefabs/9_Docking_Station_2_.prefab
- Name: AMR1T | Path: Assets/Prefabs/AMR1T.prefab
- Name: AMR300Kg | Path: Assets/Prefabs/AMR300Kg.prefab
- Name: AMR_1ton_v1 | Path: Assets/Prefabs/AMR_1ton_v1.prefab
- Name: AMR_1ton_v1_newMat | Path: Assets/Prefabs/AMR_1ton_v1_newMat.prefab
- Name: AMR_300kg_v1 | Path: Assets/Prefabs/AMR_300kg_v1.prefab
- Name: AMR_300kg_v1_newMat | Path: Assets/Prefabs/AMR_300kg_v1_newMat.prefab
- Name: ASRS_In | Path: Assets/Prefabs/ASRS_In.prefab
- Name: ASRS_Out | Path: Assets/Prefabs/ASRS_Out.prefab
...
class AgentState(TypedDict):
    ...
    scene_layout:          str                     # 씬 자동 분석 결과 (오브젝트 그룹, 페어링, 위치)
...
# ------------------------------------------------------------------
# 씬 자동 분석: hierarchy + 위치 + 근접 그룹 감지
# ------------------------------------------------------------------

SCENE_IGNORE = {"Main Camera", "Directional Light", "EventSystem", "Canvas", "Environment", "Manager"}

async def analyze_scene_layout(session) -> str:
    """
    MCP로 현재 씬을 분석하여 오브젝트 배치와 페어링 그룹을 자동 감지.
    프로젝트에 종속되지 않는 general한 분석 — 근접 거리와 부모-자식 관계만 사용.
    """
    # 1. Hierarchy 가져오기
    try:
        ...
        print(f"  [SceneAnalysis] hierarchy 조회 실패: {e}")
        return "씬 분석을 수행할 수 없습니다."

    # 2. 루트 오브젝트에서 의미있는 그룹 추출
    root_objects = []
    ...
    # 분석할 그룹 선별 (IGNORE 제외, 자식이 있는 것 위주)
    groups = {}
    for obj in root_objects:
        name = obj.get("name", "")
        if name in SCENE_IGNORE or not obj.get("children"):
            continue
        children = [c.get("name", "") for c in obj.get("children", [])]
        groups[name] = children

    # 3. 주요 오브젝트 위치 조회 (동적 설비 그룹)
    objects_info = []
    for parent_name, children in groups.items():
        for child_name in children:
            ...
            try:
                ...
                # Transform, GameObject 같은 기본 컴포넌트 제외
                meaningful_comps = [c for c in components if c not in ("Transform", "GameObject", "MeshFilter", "MeshRenderer")]
                obj_children = [c.get("name", "") for c in go.get("children", [])]
                ...
                objects_info.append({
                    ...
                    "parent": parent_name,
                    "position": position,
                    "components": meaningful_comps,
                    "children": obj_children,
                })
            except Exception:
                pass  # 일부 오브젝트 조회 실패는 무시

    # 4. 근접 그룹 감지 (distance < 5.0)
    PROXIMITY_THRESHOLD = 5.0
    pairs = []
    for i, a in enumerate(objects_info):
        for j, b in enumerate(objects_info):
            if i >= j:
                continue
            dx = a["position"].get("x", 0) - b["position"].get("x", 0)
            dz = a["position"].get("z", 0) - b["position"].get("z", 0)
            dist = math.sqrt(dx * dx + dz * dz)
            if 0.01 < dist < PROXIMITY_THRESHOLD:
                pairs.append((a["name"], b["name"], round(dist, 2), a["parent"], b["parent"]))

    # 5. 클러스터링 (근접한 오브젝트들을 그룹으로 묶기)
    adjacency = {}
    for a_name, b_name, dist, _, _ in pairs:
        if dist < 2.0:  # 매우 가까운 것들만 클러스터링
    ...
    # 6. 결과 텍스트 생성
    lines = []
    lines.append("[씬 오브젝트 배치 분석]")
    lines.append("")

    # 부모-자식 그룹
    lines.append("== 씬 구조 (부모 → 자식) ==")
    for parent_name, children in groups.items():
        lines.append(f"  {parent_name}: {children}")
    lines.append("")

    # 오브젝트 위치
    lines.append("== 주요 오브젝트 위치 ==")
    for o in objects_info:
        px = o["position"].get("x", 0)
        pz = o["position"].get("z", 0)
        comps = o["components"][:3]
        comp_str = f" [{', '.join(comps)}]" if comps else ""
        lines.append(f"  {o['name']:35s} x={px:7.2f}  z={pz:7.2f}{comp_str}")
    lines.append("")

    # 페어링 그룹 (클러스터)
    lines.append("== 자동 감지된 페어링 그룹 (근접 거리 기반) ==")
    lines.append("아래 그룹의 오브젝트들은 함께 배치되어야 합니다:")
    for i, cluster in enumerate(clusters, 1):
        # 각 클러스터 멤버의 컴포넌트 정보 추가
        members = []
        for name in cluster:
            info = next((o for o in objects_info if o["name"] == name), None)
            if info and info["components"]:
                members.append(f"{name} [{', '.join(info['components'][:2])}]")
            else:
                members.append(name)
        lines.append(f"  그룹 {i}: {', '.join(members)}")
    lines.append("")

    # 제약조건 요약
    lines.append("== 배치 제약조건 ==")
    lines.append("1. 위 페어링 그룹의 오브젝트들은 반드시 가까이 (distance < 2.0) 배치할 것")
    lines.append("2. 같은 부모 아래 오브젝트들은 논리적으로 관련 있음")
    lines.append("3. 새 오브젝트 배치 시 기존 페어링 그룹을 깨뜨리지 말 것")

    result_text = "\n".join(lines)
    print(f"  [SceneAnalysis] {len(objects_info)}개 오브젝트, {len(clusters)}개 페어링 그룹 감지")
    return result_text
...
{state.get('scene_layout', '')}

작성 규칙:
1. 씬에 이미 존재하는 오브젝트는 다시 배치하지 마.
1-1. [중요] 위 "자동 감지된 페어링 그룹"의 오브젝트들은 반드시 가까이 배치해야 한다. 새 배치가 기존 페어링을 깨뜨리면 안 됨.
...
async def init_node(state: AgentState, config) -> dict:
    """MCP 도구 목록, 씬 상태, 씬 레이아웃 분석 + 사용자 요청 입력"""
    ...
    print("\n씬 레이아웃 분석 중...")
    scene_layout = await analyze_scene_layout(session)
    print(f"\n{scene_layout}\n")
...
async def search_prefabs_graph_node(state: AgentState, config) -> dict:
    """Prefab 목록 반환 (고정 목록 사용)"""
    print(f"\n[사용 가능한 Prefab 목록]\n{FIXED_PREFABS}")
    return {"available_prefabs": FIXED_PREFABS}
